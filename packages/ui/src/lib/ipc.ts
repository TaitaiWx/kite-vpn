...
export async function testNodeRealSpeed(
  ...
  const result = await invoke<RustResult>("test_node_real_speed", {
    name,
    mode,
    timeoutMs,
  });
...
export async function fetchRemoteSubscription(
  url: string,
  timeoutMs?: number,
): Promise<IpcResult<FetchedSubscription>> {
  return invoke<FetchedSubscription>("fetch_remote_subscription", {
    url,
    timeoutMs,
  });
}
...
export async function mihomoGetLogs(
  sinceIndex?: string,
): Promise<IpcResult<LogChunk>> {
...
export async function mihomoReloadConfig(
  configPath?: string,
): Promise<IpcResult<void>> {
  return invoke<void>("mihomo_reload_config", { configPath });
}

export async function mihomoSelectProxy(
  group: string,
  proxy: string,
): Promise<IpcResult<void>> {
...
export async function meshGetEngineState(): Promise<
  IpcResult<MeshEngineState>
> {
...
export async function meshCreateNetwork(
  ...
  return invoke<MeshNetwork>("mesh_create_network", {
    name,
    lighthouseEndpoint,
  });
}
...
export async function meshJoinNetwork(
  token: string,
): Promise<IpcResult<MeshNetwork>> {
...
export async function accountSetServer(
  serverUrl: string,
): Promise<IpcResult<AccountState>> {
...
export async function accountRequestLogin(
  email: string,
): Promise<IpcResult<void>> {
  return invoke<void>("account_request_login", { email });
}

export async function accountRememberEmail(
  email: string,
): Promise<IpcResult<void>> {
  return invoke<void>("account_remember_email", { email });
}

export async function accountVerifyLogin(
  token: string,
): Promise<IpcResult<AccountState>> {
...
export async function accountCreateInvite(
  request: CreateInviteRequest,
): Promise<IpcResult<CreatedInvite>> {
...
export async function accountRevokeInvite(
  slug: string,
): Promise<IpcResult<void>> {
...
export async function accountConsumeInvite(
  ...
  return invoke<ConsumedInvite>("account_consume_invite", {
    serverUrl,
    slug,
    passphrase,
  });
...
export async function accountCreateBridgeInvite(
  ...
  return invoke<CreatedBridgeInvite>("account_create_bridge_invite", {
    request,
  });
}

export async function accountRedeemBridge(
  request: RedeemBridgeRequest,
): Promise<IpcResult<RedeemedBridge>> {
...
export async function accountRevokeBridge(
  id: string,
): Promise<IpcResult<void>> {
...
export async function accountFetchUpdatePubkey(): Promise<
  IpcResult<UpdatePubkeyInfo>
> {
  return invoke<UpdatePubkeyInfo>("account_fetch_update_pubkey", {});
}

export async function accountCheckUpdateSigned(): Promise<
  IpcResult<SignedUpdateCheck>
> {
...
function makeNode(
  ...
  switch (protocol) {
    ...
    case "trojan":
      return {
        ...base,
        settings: { protocol: "trojan", password: "mock-trojan" },
      };
    case "hysteria2":
      return {
        ...base,
        settings: { protocol: "hysteria2", password: "mock-hy2" },
      };
    case "tuic":
      return {
        ...base,
...
const MOCK_GROUPS: ProxyGroupConfig[] = [
  { name: "Proxy", type: "select", proxies: MOCK_NODES.map((n) => n.name) },
  {
    name: "Auto - HK",
    type: "url-test",
    proxies: MOCK_NODES.filter((n) => n.region === "HK").map((n) => n.name),
  ...
  },
  {
    name: "Auto - JP",
    type: "url-test",
    proxies: MOCK_NODES.filter((n) => n.region === "JP").map((n) => n.name),
    url: "http://www.gstatic.com/generate_204",
    interval: 300,
    tolerance: 50,
  },
...
const MOCK_SUBSCRIPTIONS: Subscription[] = [
  {
    id: "sub-001",
    name: "CloudFlare Pro",
    url: "https://example.com/sub/cloudflare?token=abc123",
    enabled: true,
    nodes: MOCK_NODES.slice(0, 4),
    lastUpdate: new Date("2025-06-10T14:30:00Z"),
    updateIntervalHours: 12,
    userInfo: {
      upload: 12_580_000_000,
      download: 89_420_000_000,
  ...
  },
  {
    id: "sub-002",
    name: "SpeedNet",
    url: "https://speednet.io/api/subscribe?key=xyz789",
    enabled: true,
    nodes: MOCK_NODES.slice(4),
    lastUpdate: new Date("2025-06-10T10:15:00Z"),
    updateIntervalHours: 24,
    userInfo: {
      upload: 3_200_000_000,
      download: 45_800_000_000,
...
function makeMockConnection(index: number): ConnectionInfo {
  ...
  const metadata: ConnectionMetadata = {
    network: index % 3 === 0 ? "udp" : "tcp",
    type: index % 4 === 0 ? "HTTP" : "HTTPS",
    sourceIP: "127.0.0.1",
    sourcePort: String(50000 + index),
    destinationIP: `104.${16 + (index % 8)}.${index % 256}.${(index * 7) % 256}`,
    destinationPort: index % 3 === 0 ? "80" : "443",
    host,
    process: index % 2 === 0 ? "chrome" : "curl",
  };
  return {
    id: `conn-${String(index).padStart(4, "0")}`,
    metadata,
    chains: chain,
    rule,
    rulePayload: host,
    start: new Date(Date.now() - index * 60_000).toISOString(),
    upload: Math.floor(Math.random() * 500_000),
    download: Math.floor(Math.random() * 5_000_000),
  };
}
...
const LOG_MESSAGES: Array<{ type: LogEntry["type"]; payload: string }> = [
  {
    type: "info",
    payload:
      "[TCP] 127.0.0.1:52341 --> www.google.com:443 match GEOSITE using Proxy[🇭🇰 Hong Kong 01]",
  },
  {
    type: "info",
    payload:
      "[TCP] 127.0.0.1:52342 --> api.github.com:443 match DOMAIN-SUFFIX using Proxy[🇯🇵 Tokyo 01]",
  },
  {
    type: "debug",
    payload: "[DNS] resolve www.youtube.com -> fake-ip 198.18.0.42",
  },
  {
    type: "warning",
    payload: "[TUN] high packet loss detected on interface utun3",
  },
  {
    type: "error",
    payload:
      "[Engine] failed to dial 🇮🇳 Mumbai 01: connection timeout after 5000ms",
  },
];
...
export function getMockTraffic(): TrafficStats {
  return mockTraffic();
}
...
export function getMockEngineState(): EngineState {
  return { status: "stopped" };
}
export function getMockNodes(): ProxyNode[] {
  return [...MOCK_NODES];
}
export function getMockGroups(): ProxyGroupConfig[] {
  return MOCK_GROUPS.map((g) => ({ ...g, proxies: [...g.proxies] }));
}
export function getMockSubscriptions(): Subscription[] {
  return MOCK_SUBSCRIPTIONS.map((s) => ({ ...s, nodes: [...s.nodes] }));
}
...
export function getMockAppConfig(): AppConfig {
  return structuredClone(MOCK_APP_CONFIG);
}
...
async function mockInvoke(
  ...
  switch (command) {
    case "engine_start":
      return { success: true, data: getMockEngineState() };
    case "engine_stop":
    ...
    case "engine_get_state":
      return { success: true, data: getMockEngineState() };
    case "write_config":
      return { success: true, data: "/mock/config.yaml" };
    case "read_config":
      return { success: true, data: "" };
    case "save_subscriptions":
      return { success: true, data: undefined };
    case "load_subscriptions":
      return { success: true, data: JSON.stringify(MOCK_SUBSCRIPTIONS) };
    case "save_app_config":
      return { success: true, data: undefined };
    case "load_app_config":
      return { success: true, data: JSON.stringify(MOCK_APP_CONFIG) };
    case "enable_system_proxy":
      return { success: true, data: true };
    case "disable_system_proxy":
      return { success: true, data: false };
    case "get_system_proxy_status":
      return { success: true, data: false };
    case "set_mode":
      return { success: true, data: undefined };
    case "test_proxy_delay":
      return {
        success: true,
        data: { name: "test", delay: Math.floor(Math.random() * 300) + 20 },
      };
    case "fetch_remote_subscription":
      return { success: false, error: "浏览器开发模式不支持远程拉取" };
    case "mihomo_get_traffic":
      return {
        success: true,
        data: {
          up: Math.floor(Math.random() * 500000),
          down: Math.floor(Math.random() * 5000000),
        },
      };
    case "mihomo_get_connections":
      return {
        success: true,
        data: JSON.stringify({
          connections: [],
    ...
    case "mihomo_get_proxies":
      return { success: true, data: JSON.stringify({ proxies: {} }) };
    case "mihomo_get_rules":
      return {
        success: true,
        data: JSON.stringify({
          rules: getMockRules().map((r, i) => ({
            type: r.type,
            payload: r.payload,
            proxy: r.target,
            size: -1,
    ...
    case "mihomo_get_logs":
      return { success: true, data: { lines: [], total: 0 } };
    case "mihomo_get_version":
      return { success: true, data: "v1.18.0" };
    case "mihomo_reload_config":
      return { success: true, data: undefined };
    case "mihomo_select_proxy":
      return { success: true, data: undefined };
    case "mihomo_close_connections":
      return { success: true, data: undefined };
    default:
