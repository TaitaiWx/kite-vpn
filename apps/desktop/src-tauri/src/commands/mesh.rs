...
/// 一条跨 Mesh 桥规则 —— 来自 backend /api/bridges 的 active 记录。
/// 落到 nebula firewall 里：允许指定 ca_sha 的对端访问本机指定端口。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BridgeRule {
    pub direction: String,         // "in" / "out" / "both"
    pub remote_ca_sha256: String,  // 32 hex 字符
    pub local_peer_id: String,     // 本机要暴露的 peer（用 host 字段匹配）
}
...
fn render_node_config(
    ...
    render_node_config_with_bridges(mesh_dir, self_cert_name, lighthouse_endpoint, &[])
}

/// 带 bridge firewall 规则的版本。每条 bridge 加 inbound / outbound 规则。
pub(super) fn render_node_config_with_bridges(
    mesh_dir: &Path,
    self_cert_name: &str,
    lighthouse_endpoint: &str,
    bridges: &[BridgeRule],
    ...
    // 默认 firewall：允许同 CA 的所有 peer ICMP + 任意。
    // 跨 mesh bridge：每条加 inbound 规则用 ca_sha 限定来源 CA
    let mut bridge_inbound = String::new();
    let mut bridge_outbound = String::new();
    for b in bridges {
        if b.direction == "in" || b.direction == "both" {
            bridge_inbound.push_str(&format!(
                "    - port: any\n      proto: any\n      ca_sha: {}\n",
                b.remote_ca_sha256
            ));
        }
        if b.direction == "out" || b.direction == "both" {
            bridge_outbound.push_str(&format!(
                "    - port: any\n      proto: any\n      ca_sha: {}\n",
                b.remote_ca_sha256
            ));
        }
        let _ = &b.local_peer_id; // v1 firewall 按 CA 维度限制，未来 v1.1 加精确到 host 的 rule
    }

    format!(
        r#"# Generated by Kite — Phase 4/5 Mesh (含 cross-mesh bridge firewall 规则)
pki:
  ca: "{ca}"
  cert: "{crt}"
  key: "{key}"
...
// ─── G: 应用 cross-mesh bridge firewall 规则 ──────────────────────────────

/// IPC: 从 backend 拉当前 active bridges → 重渲染本机 nebula config（加 firewall
/// ca_sha 规则）→ 写盘到 mesh/bridges.json 让 UI 可见。
///
/// **不**自动重启 nebula —— 用户在 UI 上看到生效列表后手动点 "重启 Mesh" 应用。
/// 这是出于谨慎：firewall 改动是不可见的，自动重启可能让用户莫名其妙断连。
#[tauri::command]
pub async fn mesh_apply_bridges(app: AppHandle) -> super::IpcResult<usize> {
    let dir = match mesh_dir(&app) {
        Ok(d) => d,
        Err(e) => return super::IpcResult::err(e),
    };
    let network_path = dir.join("network.json");
    if !network_path.exists() {
        return super::IpcResult::err("尚未加入 Mesh 网络".to_string());
    }

    // 从 account_bridges 模块拉 backend
    let bridges = match super::account_bridges::fetch_active_bridges_for_mesh(&app).await {
        Ok(b) => b,
        Err(e) => return super::IpcResult::err(e),
    };

    // 转换成 firewall rules
    let rules: Vec<BridgeRule> = bridges
        .iter()
        .filter(|b| b.status == "active" && !b.remote_ca_fingerprint.is_empty())
        .map(|b| BridgeRule {
            direction: b.direction.clone(),
            remote_ca_sha256: b.remote_ca_fingerprint.clone(),
            local_peer_id: b.local_peer_id.clone(),
        })
        .collect();
    let count = rules.len();
    ...
    if let Ok(json) = serde_json::to_string_pretty(&rules) {
        let _ = fs::write(dir.join("bridges.json"), json);
    }

    // 重渲染 config.yaml
    // 用本机节点名 = network.json 的 self_cert_name，fallback 到 "host"
    let network_str = fs::read_to_string(&network_path).unwrap_or_default();
    let network_val: serde_json::Value =
        serde_json::from_str(&network_str).unwrap_or(serde_json::Value::Null);
    let self_cert_name = network_val
        .get("self_cert_name")
        .and_then(|v| v.as_str())
        .unwrap_or("host")
        .to_string();
    let lighthouse_endpoint = network_val
        .get("lighthouse_endpoint")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();

    let yaml = render_node_config_with_bridges(&dir, &self_cert_name, &lighthouse_endpoint, &rules);
    if let Err(e) = fs::write(dir.join("config.yaml"), yaml) {
        return super::IpcResult::err(format!("写 config.yaml 失败: {}", e));
    }

    super::IpcResult::ok(count)
}
